...
// Single-file SPA + JSON API for Expenses, Accounts, Stats, Summary, Forecast
// Works with configdb.php and fnb schema
// -----------------------------------------------------------------------------
...
// fallback to demo session if login system not set up
...
if (isset($_GET['action'])) {
    ...
    if ($a === 'add' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!$eid || !$did || !$amount || !$category || !$reason || !$date) {
    ...
    if ($a === 'update' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST['date']))     { $set[]="date=?";     $types.="s"; $params[]=$_POST['date']; }
        ...
        if (empty($set)) json_out(['ok'=>false, 'error'=>'No fields to update'], 422);
    ...
    if ($a === 'summary') {
        ...
        foreach ($rows as &$r) {
            $r['share_percent'] = $grand > 0 ? round(($r['total_amount'] * 100.0) / $grand, 2) : 0.0;
        }
...
/* -------------------------------------------------------------------------- */
/*  If no action param => load HTML frontend (SPA)                            */
/* -------------------------------------------------------------------------- */
?>
<!DOCTYPE html>
<html lang="en">
        ...
        <div class="card">
          ...
          <div class="table-wrap mt-16">
            <table id="requests-table">
              <thead><tr><th>ID</th><th>Date</th><th>Amount</th><th>Category</th><th>Reason</th><th>Status</th><th>Actions</th></tr></thead>
  ...
  <script>
    ...
    async function loadStats(){
      try {
        ...
        $('#stat-pending').textContent = j.pending;
